...
<div class="content-wrapper">
 <section class="">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="popup_title2">Saved Advertisement</h1>
          </div>
          <!--<div class="col-sm-6">
		...
		@endif
          ...
          <div class="">
            <div class="">
			 ...
			 <!-- <a href="#" onclick="call_add_page()" style="font-color:#000000 !important; text-decoration:none;"><button class="btn btn-success" >Add User Saved Advertisement</button></a>-->
            </div>
            <!-- /.card-header -->
			<?PHP 
			...
			?>
            <div class="card-body">
              <table id="saved_users" class="table table-bordered table-striped">
                <thead>
				
                <tr>
                  <th>User</th>
                  <th>Advertisement Description</th>
                  <th>Advertisement Caption</th>
                 <!--<th style="min-width: 145px;">Action</th>-->
                </tr>
                </thead>
                <tbody>
				@foreach($u_s_ad as $user_s_ad)
				<?PHP 
	
				$status = ($user_s_ad['is_active']==1)? 'Active' : 'Inactive';
				$status_class = ($user_s_ad['is_active']==1)? 'btns-success': 'btns-danger';
			/* print_r($user);exit; */
			$user_name = "";
			foreach($user as $users)
			{
				
				if($users->user_id == $user_s_ad->user_id)
				{
			...
			$desc = "";
			$caption = "";
			foreach($adv_desc as $master_ads_desc)
			{
				if($user_s_ad->advertisement_id = $master_ads_desc->advertisement_id)
				{
					$desc = $master_ads_desc->description;
					$caption = $master_ads_desc->caption;
				}
			}
			...
			?>
                <tr>
				  ...
				  <a href="#" class="btn btn-outline-primary btn-sm">
				   <i title="Status" id="status_{{$user_s_ad->user_saved_advertisement_id}}" data="{{$user_s_ad->user_saved_advertisement_id}}" class="status_checks btn btn_w {{$status_class}}"><span class='{{$status}}'></span></i> </a>
				  <a title="View" class="btn btn-outline-primary btn-sm" href="javascript:void(0)" onclick = "single_view('{{$user_s_ad->user_saved_advertisement_id}}')"><i class="fa fa-eye fa-lg" aria-hidden="true" ></i></a>
				  <a href="#" class="btn btn-outline-primary btn-sm" onclick="call_page('{{encrypt($user_s_ad->user_saved_advertisement_id)}}')"><i class="fa fa-edit fa-lg" aria-hidden="true"></i></a>
				  <a href="{{url('user_saved_advdelete/'.encrypt($user_s_ad->user_saved_advertisement_id))}}" class="btn btn-outline-primary btn-sm"><li  class="fa fa-trash fa-lg" onclick= 'return doconfirm()'></li></a></td>-->
                </tr>
				@endforeach
                </tbody>
               <?PHP /* <tfoot>
