			  ...
			  {!! Form::open(array("enctype"=>"multipart/form-data","id"=>"demo-form","data-parsley-validate"=>"")) !!}
                <div class="card-body">
                   <div class="form-group">
					...
					<option value="{{$wal->business_id}}" @if($coupons[0]->business_id == $wal->business_id){{'selected'}}@endif>{{$wal->name}}</option>
					@endforeach
					</select>
				  ...
				  <div class="form-group">
					...
					{!!Form::text('code',$coupons[0]->code,['class'=>'form-control','placeholder'=>'Code','required'=>'','id'=>'coupon_code']) !!}
					<a class="error_1" id="code_unique">{{ $errors->first('email') }}</a>
                  </div>
				  <div class="form-group">
                    {!! Form::label('value','Value') !!}<span class="mandatory">*</span>
					{!! Form::text('value',$coupons[0]->value,['class'=>'form-control','placeholder'=>'Value','required'=>'','data-parsley-type'=>"number"]) !!}
                  </div> 
				 
					 <div class="form-group">
                    {!! Form::label('max_redemption_count','Max Redemption Count') !!}
					{!! Form::text('max_redemption_count',$coupons[0]->max_redemption_count,['class'=>'form-control','placeholder'=>'Max Redemption Count','data-parsley-type'=>"number"]) !!}
					
					<input type="text" value="{{ decrypt(Request::segment(2)) }}" id="coupon_id" hidden>
                  </div> 
                </div>
                <!-- /.card-body -->
