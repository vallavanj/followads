...
<div class="content-wrapper">
 <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Manage Gift Coupons</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
			  <li><i class="nav-icon fa fa-dashboard"></i></li>
              <li class="breadcrumb-item"><a href="{{url('dashboard')}}">Home</a></li>
              <li class="breadcrumb-item active">Manage Gift Coupons</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="card">
			...
			<?php 
			 ...
			 ?>
			  ...
			  <a href="#"  onclick="call_add_page()" style="font-color:#000000 !important; text-decoration:none;"><button class="btn btn-success" title="Add Gift Coupons" data-toggle="tooltip" data-placement="bottom" >Add Gift Coupons</button></a>
			  <?php
				}
			  ?>
            </div>
            ...
            <div class="table-responsive">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
				@foreach($deatils as $coupons)
				<?PHP 
	
				$status = ($coupons->is_active==1)? 'Active' : 'Inactive';
				$status_class = ($coupons->is_active==1)? 'btns-success': 'btns-danger';
			/* print_r($lang);exit; */
				$generated_on = date('d-m-Y', strtotime($coupons->generated_on));
				$activated_on = date('d-m-Y', strtotime($coupons->activated_on));
			...
			?>
				...
				<td>{{$i++}}</td>
                  ...
                  <td><?php  foreach($business as $buss )
						{
						if($buss->business_id == $coupons->business_id)
							{
							 echo '<a title="View" data-toggle="tooltip" data-placement="bottom" class="" href="javascript:void(0)" onclick = "single_view('.$coupons->gift_coupon_id.')">'.$bus_name = $buss->name.'<a>';
							}
						}?></td>
					
                  <td>{{$coupons->code}}</td>
                  <td>{{$coupons->value}}</td>
				  <td>{{$coupons->max_redemption_count}}</td>
                  <td> {{$generated_on}}</td>
                  <td>
				  @foreach($user as $usr)
				  @if($usr->user_id == $coupons->generated_by)
				  {{$usr->name}}
				@endif
					@endforeach
				  </td>
                 ...
                 <td>
				  @foreach($user as $usr)
				  @if($usr->user_id ==$coupons->activated_by)
				  {{$usr->name}}
				@endif
					@endforeach
				 </td>
				  <td>
				  <?php if($coupons->active == 1)
				  {
					  if($user_role != "3")
						{
				 ?>
				  <a href="#" title="Edit" data-toggle="tooltip" data-placement="bottom" class="btn btn-outline-primary btn-sm" onclick="call_page('{{encrypt($coupons->gift_coupon_id)}}')"><i class="fa fa-edit fa-lg" aria-hidden="true"></i></a>
				  <a class="btn btn-outline-primary btn-sm" title="Delete" data-toggle="tooltip" data-placement="bottom" href="{{url('couponsdelete/'.encrypt($coupons->gift_coupon_id))}}"><li  class="fa fa-trash fa-lg" onclick= 'return doconfirm()'></li></a>
				  <a href="#" title="Status" data-toggle="tooltip" data-placement="bottom" class="btn btn-outline-primary btn-sm">
					<i id="status_{{$coupons->gift_coupon_id}}" data="{{$coupons->gift_coupon_id}}" class="status_checks btn btn_w {{$status_class}}"><span class="{{$status}}"></span></i></a> 
				  <?php
						}
						else
						{
				?>
							<!--<a title="View" data-toggle="tooltip" data-placement="bottom" class="btn btn-outline-primary btn-sm" href="javascript:void(0)" onclick = "single_view('{{$coupons->gift_coupon_id}}')"><i class="fa fa-eye fa-lg" aria-hidden="true" ></i></a>-->
				<?php
						}
					?>
				  <?php
				  }else{echo "<a style='color:#ee2b01;'>Business deactivated</a>";}
				  ?>
				  </td>
                </tr>
				@endforeach
