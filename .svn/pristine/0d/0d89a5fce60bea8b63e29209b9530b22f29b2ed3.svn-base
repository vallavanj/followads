...
/* alert(status); */
		$.ajax({
					type      : "POST",
					url       : "{{URL('ads_search_activestatus')}} ",   
					data:{id:id ,
						status:status,
						_token: '{{ csrf_token() }}'
							},
		...
		@endif
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Manage Advertisement Search</h3>
            </div>
            <!-- /.card-header -->
          ...
          <div class="card">
            <div class="card-header">
			  ...
			  <a href="#" onclick="call_add_page()" style="font-color:#000000 !important; text-decoration:none;"><button class="btn btn-success" >Add Advertisement Search</button></a>
            </div>
            <!-- /.card-header -->
			...
			?>
                <tr>
                  <td>{{$ads->advertisement_id}}</td>
                  <td>{{$ads->search_term}}</td>
				  @foreach($lang as $lan)
				  @if($ads->language_id == $lan->language_id)
                  <td>{{$lan->name_in_english}}</td>
				  @endif
					@endforeach
                  <td>
				  <a href="#" class="btn btn-outline-primary btn-sm">
				  <i title="Status" id="status_{{$ads->advertisement_search_term_id}}" data="{{$ads->	advertisement_search_term_id}}" class="status_checks btn btn_w {{$status_class}}"><span class='{{$status}}'></span></i></a> 
				  <a title="View" class="btn btn-outline-primary btn-sm" href="javascript:void(0)" onclick = "single_view('{{$ads->advertisement_search_term_id}}')"><i class="fa fa-eye fa-lg" aria-hidden="true" ></i></a>
				  <a href="#" class="btn btn-outline-primary btn-sm" onclick="call_page('{{encrypt($ads->advertisement_search_term_id)}}')"><i class="fa fa-edit fa-lg" aria-hidden="true"></i></a>
				  <a href="{{url('ads_searchdelete/'.encrypt($ads->advertisement_search_term_id))}}" class="btn btn-outline-primary btn-sm"><li  class="fa fa-trash fa-lg" onclick= 'return doconfirm()'></li></a></td>
                </tr>
				@endforeach
                </tbody>
               <?PHP /* <tfoot>
