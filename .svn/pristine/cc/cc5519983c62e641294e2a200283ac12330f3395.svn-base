...
/* alert(status); */
		$.ajax({
					type      : "POST",
					url       : "{{URL('ads_offer_activestatus')}} ",   
					data:{id:id ,
						status:status,
						_token: '{{ csrf_token() }}'
							},
		...
		@endif
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Manage Advertisement Offer Code</h3>
            </div>
            <!-- /.card-header -->
          ...
          <div class="card">
            <div class="card-header">
			  ...
			  <a href="#" onclick="call_add_page()" style="font-color:#000000 !important; text-decoration:none;"><button class="btn btn-success" >Add Advertisement Offer Code</button></a>
            </div>
            <!-- /.card-header -->
			...
			?>
                <tr>
                  ...
                  <td>{{$ads->offer_code}}</td>
                  <td>{{$ads->offer_type_code}}</td>
				  
                 <!-- <td>
				  <a href="#" class="btn btn-outline-primary btn-sm">
				  <i title="Status" id="status_{{$ads->advertisement_offer_code_id}}" data="{{$ads->advertisement_offer_code_id}}" class="status_checks btn btn_w {{$status_class}}"><span class='{{$status}}'></span></i></a> 
				  <a title="View" class="btn btn-outline-primary btn-sm" href="javascript:void(0)" onclick = "single_view('{{$ads->advertisement_offer_code_id}}')"><i class="fa fa-eye fa-lg" aria-hidden="true" ></i></a>
				  <a href="#" class="btn btn-outline-primary btn-sm" onclick="call_page('{{encrypt($ads->advertisement_offer_code_id)}}')"><i class="fa fa-edit fa-lg" aria-hidden="true"></i></a>
				  <a href="{{url('ads_offerdelete/'.encrypt($ads->advertisement_offer_code_id))}}" class="btn btn-outline-primary btn-sm"><li  class="fa fa-trash fa-lg" onclick= 'return doconfirm()'></li></a></td>-->
                </tr>
				@endforeach
                </tbody>
               <?PHP /* <tfoot>
