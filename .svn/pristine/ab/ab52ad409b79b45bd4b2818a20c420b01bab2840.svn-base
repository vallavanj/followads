...
/* alert(status); */
		$.ajax({
					type      : "POST",
					url       : "{{URL('buss_cat_single_active')}} ",   
					data:{id:id ,
						status:status,
						_token: '{{ csrf_token() }}'
							},
		...
		@endif
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Manage Business Category</h3>
            </div>
            <!-- /.card-header -->
          ...
          <div class="card">
            <div class="card-header">
			  ...
			  <a href="#"  onclick="call_add_page()" style="font-color:#000000 !important; text-decoration:none;"><button class="btn btn-success" >Add BusinessCategory</button></a>
            </div>
            <!-- /.card-header -->
			<?PHP 
			...
			?>
				...
				@foreach($categ as $categr)
					@if($buss_cat->category_id == $categr->category_id)
                  <td>{{$categr->category_name}}</td>
					@endif
				@endforeach
				  <td>
				  <a href="#" class="btn btn-outline-primary btn-sm">
				  <i title="Status" id="status_{{$buss_cat->business_category_id}}" data="{{$buss_cat->business_category_id}}" class="status_checks btn btn_w {{$status_class}}"><span class="{{$status}}"></span></i> </a>
				  <a title="View" class="btn btn-outline-primary btn-sm" href="javascript:void(0)" onclick = "single_view('{{$buss_cat->business_category_id}}')"><i class="fa fa-eye fa-lg" aria-hidden="true" ></i></a>
				  <a href="#" class="btn btn-outline-primary btn-sm" onclick="call_page('{{encrypt($buss_cat ->business_category_id)}}')"><i class="fa fa-edit fa-lg" aria-hidden="true"></i></a>
				  <a href="{{url('buss_catdelete/'.encrypt($buss_cat ->business_category_id))}}" class="btn btn-outline-primary btn-sm"><li  class="fa fa-trash fa-lg" onclick= 'return doconfirm()'></li></a></td>
                </tr>
				@endforeach
                </tbody>
               <?PHP /* <tfoot>
