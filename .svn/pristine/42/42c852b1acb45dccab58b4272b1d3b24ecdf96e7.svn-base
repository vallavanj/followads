			  ...
			  {!! Form::open(array("enctype"=>"multipart/form-data")) !!}
                <div class="card-body">
                  <div class="form-group">
					...
					{!!Form::text('channel',$u_bank_details[0]->channel,['class'=>'form-control','placeholder'=>'Channel']) !!}
					<a class="error_1">{{ $errors->first('channel') }}</a>
                  </div>
				  <div class="form-group">
                    {!! Form::label('ac_holdeer_name','Account Holder Name') !!}
					{!!Form::text('ac_holdeer_name',$u_bank_details[0]->account_holder_name,['class'=>'form-control','placeholder'=>'Account Holder Name']) !!}
					<a class="error_1">{{ $errors->first('ac_holdeer_name') }}</a>
                  </div>
				  <div class="form-group">
                    {!! Form::label('code','Code') !!}<span class="mandatory">*</span>
					{!!Form::text('code',$u_bank_details[0]->code,['class'=>'form-control','placeholder'=>'Code']) !!}
					<a class="error_1">{{ $errors->first('code') }}</a>
                  </div>
                  <div class="form-group">
                    {!! Form::label('ifsc','Ifsc') !!}
					{!!Form::text('ifsc',$u_bank_details[0]->ifsc_code,['class'=>'form-control','placeholder'=>'Ifsc']) !!}
